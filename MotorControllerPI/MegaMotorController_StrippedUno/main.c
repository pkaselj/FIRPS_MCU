...
PRIVATE void do_advance_pids(void)
{
	...
	//
	//
	//stxetx_frame_t frame_pid;
	//
	//stxetx_init_empty_frame(&frame_pid);
	//
	//frame_pid.msg_type = MSG_TYPE_GO_FORWARD;
	//frame_pid.flags |= FLAG_SHOULD_ACK;
	//frame_pid.checksum = 0;
	//
	//uint8_t pids_value = (uint8_t)g_command_timer_pids;
	//uint8_t* payload = &pids_value;
	//
	//stxetx_add_payload(&frame_pid, payload, 1);
	//
	//usart_send_frame(frame_pid);
}
...
PRIVATE void do_on_command_complete(void)
{
	...
	uint8_t pids_value = (uint8_t)g_counter;
	uint8_t* payload = &pids_value;
...
int main(void)
{
    ...
    while (1) 
    {
		...
		if(g_command_timer_pids >= command_duration_pids)
		{
